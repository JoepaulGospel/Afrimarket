...
body: JSON.stringify({
contents: [{ parts: [{ text: `Analyze this product and return ONLY JSON: {"name": "string", "price_usd": number, "image_url": "string", "weight_kg": number, "category": "GADGETS"|"CLOTHING", "is_restricted": boolean}. Text: ${pageText}` }] }]
})
});
const aiData = await aiResponse.json();
const resultText = aiData.candidates[0].content.parts[0].text;
const cleanJson = resultText.match(/\{[\s\S]*\}/);
if (!cleanJson) throw new Error("AI Error");
