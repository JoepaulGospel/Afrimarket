...
setError("AI scan failed. The link might be protected. Try another.");
} finally {
setLoading(false);
}
};
return (
<main className="min-h-screen bg-white text-[#1d1d1f] antialiased selection:bg-indigo-100">
{/* STEVE JOBS MINIMALIST NAV */}
...
<p className="text-xs text-zinc-300 font-medium">Bypassing security protocols...</p>
</div>
</div>
)}
{product && !loading && (
...
{/* PRODUCT VISUAL */}
<div className="relative bg-[#f5f5f7] rounded-[4rem] p-12 aspect-square flex items-center justify-center overflow-hidden border border-black/5 shadow-inner">
<img src={product.image_url || "https://images.unsplash.com/photo-1587202372775-e229f172b9d7?w=1000"} className="w-full h-full object-contain mix-blend-multiply transition-transform duration-1000 hover:scale-110" alt="Product" />
<button onClick={() => setProduct(null)} className="absolute top-8 right-8 p-3 bg-white/80 backdrop-blur-md rounded-full hover:bg-white shadow-sm transition-colors">
<X size={20}/>
</button>
</div>
{/* PRODUCT DETAILS */}
...
<CheckCircle size={12}/> AI Verified Global Link
</div>
...
<ShieldAlert size={24}/> <span className="uppercase tracking-widest text-xs font-black tracking-[0.2em]">Security Block</span>
</div>
<p className="text-sm text-red-700/70 leading-relaxed font-medium">
This item contains restricted materials (batteries, liquids, or chemicals) and cannot be shipped via air freight to Nigeria.
</p>
</div>
) : (
...
<span className="text-zinc-400">International Value</span>
<span className="text-slate-900 font-bold italic">₦{product.pricing.itemNgn.toLocaleString()}</span>
</div>
<div className="flex justify-between text-base font-medium">
<span className="text-zinc-400">Freight & Logistics ({product.origin})</span>
<span className="text-slate-900 font-bold italic">₦{product.pricing.shipNgn.toLocaleString()}</span>
</div>
...
<span className="text-5xl font-black tracking-tighter text-indigo-600 animate-pulse">₦{product.pricing.total.toLocaleString()}</span>
</div>
</div>
<div className="space-y-4">
<button
onClick={() => {
const msg = `AFRIMARKET ORDER\n----------\nPRODUCT: ${product.name}\nLINK: ${url}\nTOTAL: ₦${product.pricing.total.toLocaleString()}`;
window.location.href = `https://wa.me/234XXXXXXXXXX?text=${encodeURIComponent(msg)}`;
}}
className="w-full py-7 bg-black text-white rounded-[2.5rem] font-bold text-2xl hover:scale-[1.02] shadow-2xl flex items-center justify-center gap-4 transition-all active:scale-95"
>
